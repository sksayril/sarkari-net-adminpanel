...
import { UpdateSubCategoryRequest, SubCategory } from '../api';
...
interface EditSubCategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, categoryData: UpdateSubCategoryRequest) => Promise<void>;
  isLoading: boolean;
...
const EditSubCategoryModal: React.FC<EditSubCategoryModalProps> = ({
  ...
  useEffect(() => {
    if (subCategory) {
      setFormData({
        ...
        isActive: true // Default to true, you might want to add this field to your API
      });
      setKeywordInput('');
      setTagInput('');
      setErrors({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || !subCategory) {
      return;
    }

    try {
      await onSubmit(subCategory._id, formData);
      handleClose();
    } catch (error) {
  ...
  const handleInputChange = (field: keyof UpdateSubCategoryRequest, value: string | boolean) => {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Edit Sub Category</h3>
              <p className="text-sm text-gray-600">Update sub category information</p>
            </div>
          </div>
          <button
            onClick={handleClose}
